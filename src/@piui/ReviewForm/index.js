...
import { Select, MenuItem, Box, Typography, TextField, Button } from '@material-ui/core';
import { Table, Row, Cell } from '@piui';
import { Header, Actions } from '@piui/Review';
...
const ReviewForm = ({ comment, ...props }) => {
  const [ data, setData ] = React.useState({ ...defaultData });
  const isComplete = Object.values(data).filter(d=>d===0).length>0;
  ...
  return (<>
    ...
    { (props.getReturn && !Boolean(comment)) && (<Actions mt={5}>
      <Button variant="outlined" color="primary" onClick={()=>props.getReturn(data)} disabled={isComplete}>Submit</Button>
    </Actions>)  }
  </>)
}
